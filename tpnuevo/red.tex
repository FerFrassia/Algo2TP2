...
\begin{enumerate}
	\item{claves($directasEInterfaces$) es igual que claves($deOrigenADestino$) y también que el conjunto formado por las ip de $computadoras$.}
	\item{Si def?(c, $directas$) de algún Obtener de $directasEInterfaces$ entonces c pertenece al conjunto formado por las ip de $computadoras$.}
	\item{Para todo c, si def?(c, $directasEInterfaces$), entonces no def?(c, obtener(c, $directasEInterfaces$).$directas$).}
	\item{El conjunto formado por las ip de $compusDirectas$ es el mismo que el formado por claves($directas$).}
	\item{Los Obteners de $directas$ son únicos.}	
	\item{Si def?(c, $indirectas$) de algún Obtener de $deOrigenADestino$ entonces c pertenece al conjunto formado por las ip de $computadoras$.}
	\item{Para todo c, si def?(c, $deOrigenADestino$), entonces no def?(c, obtener(c, $deOrigenADestino$)).}
