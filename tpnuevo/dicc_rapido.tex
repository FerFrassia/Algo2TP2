...
\begin{Interfaz}
 ...
 \textbf{parámetros formales}: $\alpha$, $\beta$; $\alpha$ tiene relación de orden
  ...
  [true]
  {$res$ $\igobs$ def?($c, d$)}
  [\Complejidad{log_2\ n * k}, siendo n la cantidad de claves y k el costo de comparación de $\alpha$]
 ...
 %[$\Complejidad{n * max(log_2\ n * k + copy(\beta), n)}$, siendo n la cantidad de claves del diccionario y k el costo de comparación de $\alpha$.]
 ...
 [Res es modificable si y sólo si el Diccionario es modificable]
 ...
 {$p$ $\igobs$ rotar($p_0$)}
 [$\Complejidad{k}$, siendo k el costo de comparación de *(p).clave.]
 ...
 [Calcula y devuelve el factor de desbalance actual de p]
  ...
  [Devuelve la lista de nodos no iterados hasta el momento. Están ordenados por orden de aparición.]
...
\begin{enumerate}
	...
	\item{No hay ciclos. Más formalmente, partiendo de un nodo, no se puede volver a pasar por él sin recurrir a la componente padre.}
\end{enumerate}
...
\begin{enumerate}
	...
	\item{it.raiz apunta a la raíz del AVL que itera, o es NULL si el AVL es vacío.}
	\item{it.nodoActual apunta a un nodo cualquiera del AVL que itera, o es NULL si el AVL es vacío.}
	\item{it.$\#$nodosRecorridos está entre 0 (inclusive) y it.tam (inclusive).}
	\item{it.nivelActual está entre 1 (inclusive) y $log_2\ it.tam$ + 1 (inclusive).}
...
\\ -El tercer caso es cuando se quiera definir una clave que no estaba definida anteriormente. Nuevamente ignoraremos las asignaciones, y nos centraremos en los dos ciclos. El primer ciclo consiste en iterar hasta llegar a la posición donde queremos insertar el nuevo nodo, nuevamente esto es \Complejidad{log_2\ n * k} porque en peor caso tendríamos que descender hasta la hoja más lejana (\Complejidad{log_2\ n} iteraciones, y cada iteración cuesta \Complejidad{k}). Por último, el segundo ciclo recorre, de abajo hacia arriba, la rama por la que acabamos de bajar, y dado que como ya mencionamos que ésta tiene \Complejidad{log_2\ n} elementos, esto es \Complejidad{log_2\ n * k}. Finalmente, dado que este caso tiene éstas tres complejidades no anidadas: \Complejidad{log_2\ n * k}+\Complejidad{log_2\ n * k}+\Complejidad{log_2\ n * k} = 3 * \Complejidad{log_2\ n * k} = \Complejidad{log_2\ n * k}.
\\ -Ahora, como teníamos tres casos, la complejidad es el máximo de ellos: max(\Complejidad{1},\Complejidad{log_2\ n * k},\Complejidad{log_2\ n * k}) = \Complejidad{log_2\ n * k}
}
...
\Algoritmo{IBorrar}{\In{c}{$\alpha$}, \Inout{d}{diccRapido($\alpha$,$\beta$)}}{}{
	...
	\While{pNodo != NULL} \ComplejidadDer{log_2\ n * k}
		...
		\State pNodo $\gets$ padrePNodo \ComplejidadDer{1}
	\EndWhile		
}{\Complejidad{log_2\ n * k}}{Siendo n la cantidad de nodos y k el costo de comparación de $\alpha$. 
...
\\ -El primer ciclo consiste en buscar el elemento a borrar, dado que es una búsqueda en un AVL, esto es \Complejidad{log_2\ n * k}.
\\ -El segundo ciclo sucede sólo cuando el elemento a borrar tiene dos subárboles hijos distintos de NULL, esto consiste en buscar el sucesor in-order (es decir, bajar un nodo a la derecha, y luego bajar lo máximo posible hacia la izquierda. Así se encuentra el siguiente ''inmediato''). Dado que es una búsqueda, y se empieza a descender desde el nodo a borrar (en peor caso, se empieza desde la raíz), esto toma \Complejidad{log_2\ n} (porque sólo se baja hacia la izquierda, y no hay comparaciones de tipo $\alpha$). Cabe aclarar que éste ciclo no siempre se ejecuta, pero dado que en los demás casos la complejidad es de \Complejidad{k}, podemos asumir que dado el caso que haya sido, estará acotado por la complejidad del peor, osea éste.
\\ -El tercer ciclo consiste en recorrer, de abajo hacia arriba, la rama en la cual se borró el nodo auxiliar buscado (e ir rotando según corresponda), esto toma \Complejidad{log_2\ n * k}, porque se itera \Complejidad{log_2\ n}veces y cada iteración cuesta \Complejidad{k}.
...
%	\State var conj: conjLineal $\gets$ \NombreFuncion{Vacío()} \ComplejidadDer{1}
...
%}{\Complejidad{n * max(log_2\ n * k + copy(\beta), n)}}{Siendo n la cantidad de claves y k el costo de comparación de $\alpha$. Para analizar la complejidad de este algoritmo, ignoraremos las asignaciones ya que ocurren en tiempo constante y nos centraremos en el ciclo. Sabemos que cada iteración cuesta \Complejidad{max(log_2\ n * k + copy(\beta), n)}, esto podría acotarse por n, pero eso podría no siempre ser cierto (por ejemplo, cuando n sea ínfimo), por eso no lo acotaremos de esa forma. Luego el ciclo itera \Complejidad{n} veces, porque recorre todos los nodos para agregar sus significados al conjunto. Finalmente, como itera \Complejidad{n} veces y cada iteración cuesta \Complejidad{max(log_2\ n * k + copy(\beta), n)}, se tiene que la complejidad del algoritmo es \Complejidad{n * max(log_2\ n * k + copy(\beta), n)}.
%}
...
\Algoritmo{IDameNodos}{\In{p}{puntero(nodo)}, \In{actual}{nat}, \In{destino}{nat}}{Conj(nodo)}{
	...
	\Else
		...
		\EndIf
	\EndIf
}{\Complejidad{n}}{Siendo n la cantidad de nodos. Vamos a ignorar las asignaciones y condicionales, porque tienen complejidad constante. Luego para justificar la complejidad de la recursión, se podría utilizar el \NombreFuncion{Teorema Maestro}, si fuese que el árbol está completamente lleno. De esta forma, siempre se divide en dos subproblemas y siempre se hace recursión sobre esos dos subproblemas. Aplicando el teorema maestro se tiene que: se divide en dos subproblemas (c = 2), se hace recursión sobre dos subproblemas (a = 2) y el costo de las funciones auxiliares es $\theta$(n) (f(n) = $\theta$(n)), y luego se tiene que $log_c\ a$ = $log_2\ 2$ = 1). Finalmente se tiene que se cumple el segundo caso del \NombreFuncion {Teorema Maestro}, porque $\theta$(n) $\in$ $\theta$($n^{1}$), entonces la complejidad es $\theta$($n log_2\ n$). 
}	
...
\Algoritmo{IRotar}{\Inout{p}{puntero(nodo)}}{}{
	...
	\Else
		...
		\EndIf
	\EndIf
}{\Complejidad{k}}{Siendo k el costo de comparación de $\alpha$
}
