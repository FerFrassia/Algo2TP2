...
\begin{Interfaz}
  ...
  \InterfazFuncion{Vacía?}{\In{cp}{colaP(Paquete)}}{Bool}
  %[ACA VA EL PRE SI LO HUBIERA]
  {$res$ $\igobs$ vacía?($cp$)}
  [$\Complejidad{1}$]
  [verifica si una lista esta vacía]

  \InterfazFuncion{próximo}{\In{cp}{colaP(Paquete)}}{Paquete}
  [$\neg$vacía?($cp$)]
  {$res$ $\igobs$ próximo(cp)}
  [$\Complejidad{log_2\ k}$]

  \InterfazFuncion{Desencolar}{\In{cp}{colaP(Paquete)}}{colaP(Paquete)}
  [$c$ $\igobs$ $c_0$ $\wedge$ $\neg$(vacia?(c))]
  {$res$ $\igobs$ próximo($c_0$) $\wedge$ $c$ $\igobs$ desencolar($c_0$)}
  [$\Complejidad{1}$]
