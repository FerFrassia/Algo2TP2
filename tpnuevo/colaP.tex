...
\tadAxioma{suCamino($p_1$, encolar($p_2$, cp))}{\IF $p_1$ = $p_2$ THEN $\textless$$\textgreater$ ELSE suCamino(p1, cp) FI}
\tadAxioma{suCamino($p_1$, agCompu($p_2$, c, cp))}{\IF $p_1$ = $p_2$ THEN suCamino(p1, cp) $\circ$ c ELSE suCamino(p1, cp) FI}
...
\begin{Interfaz}
  ...
  [Agrega un paquete a la cola.]
	...
	\InterfazFuncion{Vacia?}{\In{c}{colaP(Paquete)}}{bool}
  %[ACA VA EL PRE (SI LO HUBIERA)]
  {$res \igobs$ vacia?(c)}
  [$\Complejidad{1}$]
  [Revisa si la cola contiene por lo menos algun elemento.]
	
	\InterfazFuncion{Desencolar}{\Inout{c}{colaP(Paquete)}}{Paquete}
  [$c \igobs c_0 \wedge \neg$(vacia?(c))]
